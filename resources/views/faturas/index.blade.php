        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    {{ __('Tem a certeza de que deseja excluir esta fatura?') }}
                </div>
    ...
    <script>
        ...
        async function getToken() {
            try {
                ...
                Mytoken = data._token; // Armazena o token
        ...
        async function getUserNames() {
            try {
                ...
                if (!response.ok) {
                    throw new Error(`Erro ao buscar os nomes de utilizadores: ${response.status}`);
                }

                validUserNames = await response.json(); // Armazena os nomes de utilizadores num array
                console.log("Nomes de utilizadores válidos:", validUserNames);
            } catch (error) {
                console.error("Erro ao buscar os nomes de utilizadores:", error);
            }
        }
        ...
        async function getAllInvoices() {
            if (!Mytoken) {
                await getToken(); // Obter token se não estiver já obtido
            }

            if (!validUserNames.length) {
                await getUserNames(); // Obter nomes de utilizadores se não tiver sido feito
            }
            ...
            try {
                ...
                if (!response.ok) {
                    throw new Error(`Erro ao buscar as faturas: ${response.status}`);
                }

                const data = await response.json();
                console.log("Faturas obtidas:", data);

                // Filtrar faturas onde o nome do cliente corresponde a um nome de utilizador válido
                const filteredInvoices = data.data.filter(invoice => validUserNames.includes(invoice.name));
        ...
        function displayInvoices(invoices) {
            ...
            invoiceListDiv.innerHTML = ""; // Limpar conteúdo anterior
            ...
            invoices.forEach(invoice => {
                ...
                // Criar botões de ação (Ver, Editar, Excluir)
                const actionsTd = document.createElement("td");
                actionsTd.classList.add("text-center");

                // Botão de visualizar (ícone de olho azul)
                ...
                deleteButton.onclick = () => openDeleteModal(invoice.id); // Chamar a função para abrir o modal de exclusão
        ...
        async function deleteInvoice() {
            try {
                ...
                getAllInvoices(); // Atualizar a lista de faturas após a exclusão
        ...
        // Confirmar a exclusão quando clicar no botão de excluir no modal
